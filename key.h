...
// rotation 함수
static inline uint8_t rot(uint8_t input, uint8_t num){
    return (input << num) | (input >> (8-num));
}

static inline uint8_t fun0(uint8_t input){
    return rot(input, 1) ^ rot(input, 2) ^ rot(input, 7);
}
static inline uint8_t fun1(uint8_t input){
    return rot(input, 3) ^ rot(input, 4) ^ rot(input, 6);
}
